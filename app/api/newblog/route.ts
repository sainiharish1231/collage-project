...
export interface IComment {
  ...
  posttime: string;
  userComment: string;
  userImage: string; // Changed from `any` to `string`
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data: Item[] = await Newblog.find({}).sort({ uploadTime: -1 }).exec();
    ...
    return NextResponse.json({
      ...
      error: (error as Error).message, // Include the error message in the response
    });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      msg: "An error occurred in the POST request",
      error: (error as Error).message, // Include the error message in the response
    });
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error("Error in PUT request:", error); // Log the error
    return NextResponse.json({
      msg: "An error occurred in the PUT request",
      error: (error as Error).message, // Include the error message in the response
    });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("Error in DELETE request:", error); // Log the error
    return NextResponse.json({
